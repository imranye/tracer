...
const newToken = () => `${id().replaceAll("-", "")}${id().replaceAll("-", "")}`;
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/" && request.method === "GET") return new Response(APP.replace('<main id="app"', '<pre id="prompt" class="hidden"></pre><main id="app"').replace('if(token)load();', 'if(token){approve();load()}'), { headers: { "content-type": "text/html;charset=UTF-8" } });
    try {
      if (url.pathname === "/api/device/start" && request.method === "POST") {
        ...
        await env.DB.prepare("INSERT INTO device_requests (id,request_hash,expires_at) VALUES (?,?,?)").bind(id(), toHex(await digest(requestToken)), expiresAt).run();
        return json({ request_token: requestToken, authorize_url: `${url.origin}/?device=${encodeURIComponent(requestToken)}` });
      }
      if (url.pathname === "/api/device/poll" && request.method === "GET") {
        const requestToken = url.searchParams.get("request_token") ?? "";
        const row = await env.DB.prepare("SELECT id,token_ciphertext,expires_at FROM device_requests WHERE request_hash=?").bind(toHex(await digest(requestToken))).first<{ id: string; token_ciphertext: string | null; expires_at: string }>();
        if (!row) return json({ status: "expired" }, 410);
        if (row.expires_at < new Date().toISOString()) { await env.DB.prepare("DELETE FROM device_requests WHERE id=?").bind(row.id).run(); return json({ status: "expired" }, 410); }
        if (!row.token_ciphertext) return json({ status: "pending" });
        const token = await decrypt(row.token_ciphertext, env.TRACER_ENCRYPTION_KEY); await env.DB.prepare("DELETE FROM device_requests WHERE id=?").bind(row.id).run(); return json({ status: "approved", token });
      }
      if (url.pathname === "/api/signup" && request.method === "POST") {
        const body = await request.json() as { email?: string; name?: string; request_token?: string };
        ...
        if (body.request_token) {
          ...
          await env.DB.batch([
            env.DB.prepare("INSERT INTO access_tokens (token_hash,account_id,created_at) VALUES (?,?,?)").bind(toHex(await digest(deviceToken)), accountId, now),
            env.DB.prepare("UPDATE device_requests SET token_ciphertext=? WHERE request_hash=? AND expires_at>? ").bind(await encrypt(deviceToken, env.TRACER_ENCRYPTION_KEY), toHex(await digest(body.request_token)), now)
          ]);
        }
      ...
      if (url.pathname === "/api/device/approve" && request.method === "POST") {
        const body = await request.json() as { request_token?: string };
        if (!body.request_token || !auth.accountId) return json({ error: "Invalid request" }, 400);
        const row = await env.DB.prepare("SELECT id,expires_at FROM device_requests WHERE request_hash=?").bind(toHex(await digest(body.request_token))).first<{ id: string; expires_at: string }>();
        if (!row || row.expires_at < new Date().toISOString()) return json({ error: "Request expired" }, 410);
        const deviceToken = newToken();
        await env.DB.batch([
          env.DB.prepare("INSERT INTO access_tokens (token_hash,account_id,created_at) VALUES (?,?,?)").bind(toHex(await digest(deviceToken)), auth.accountId, new Date().toISOString()),
          env.DB.prepare("UPDATE device_requests SET token_ciphertext=? WHERE id=?").bind(await encrypt(deviceToken, env.TRACER_ENCRYPTION_KEY), row.id)
        ]);
        return json({ ok: true });
      }
...
let token=localStorage.tracer_token||'';let data;let device=new URLSearchParams(location.search).get('device');const headers=()=>({'authorization':'Bearer '+token,'content-type':'application/json'});async function signup(){let r=await fetch('/api/signup',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:document.getElementById('signup-name').value,email:document.getElementById('signup-email').value,request_token:device})});let j=await r.json();if(!r.ok){document.getElementById('signup-status').textContent=j.error||'Could not sign up.';return}token=j.token;localStorage.tracer_token=token;document.getElementById('signup-status').textContent='Vault created. Your device is connected.';await approve();await load()}async function connect(){token=document.getElementById('token').value;localStorage.tracer_token=token;await approve();await load()}async function approve(){if(device)await fetch('/api/device/approve',{method:'POST',headers:headers(),body:JSON.stringify({request_token:device})})}async function load(){let r=await fetch('/api/state',{headers:headers()});if(!r.ok){document.getElementById('app').classList.add('hidden');return}data=await r.json();document.getElementById('app').classList.remove('hidden');let s=document.getElementById('environment');s.innerHTML=data.environments.map(e=>'<option value="'+e.id+'">'+e.name+'</option>').join('');render()}function render(){let e=document.getElementById('environment').value;let vs=data.variables.filter(v=>v.environment_id===e);document.getElementById('vars').innerHTML=vs.length?vs.map(v=>'<span class="pill">'+v.name+'</span>').join(''):'<p class="muted">No variables yet.</p>';document.getElementById('prompt').textContent='tracer run '+e+' -- codex'}document.getElementById('environment').onchange=render;async function copyPrompt(){await navigator.clipboard.writeText(document.getElementById('prompt').textContent);document.getElementById('copy').textContent='Copied'}async function save(){let r=await fetch('/api/variables',{method:'POST',headers:headers(),body:JSON.stringify({environment_id:document.getElementById('environment').value,name:document.getElementById('name').value,value:document.getElementById('value').value})});document.getElementById('status').textContent=r.ok?'Saved securely.':'Could not save variable.';if(r.ok){document.getElementById('name').value='';document.getElementById('value').value='';await load()}}async function importEnv(){let f=document.getElementById('envfile').files[0];if(!f)return;let lines=(await f.text()).split(/\\r?\\n/).filter(x=>x.trim()&&!x.trim().startsWith('#'));let failed=0;for(let line of lines){let i=line.indexOf('=');if(i<1)continue;let name=line.slice(0,i).trim();let value=line.slice(i+1).trim().replace(/^(["'])(.*)\\1$/,'$2');let r=await fetch('/api/variables',{method:'POST',headers:headers(),body:JSON.stringify({environment_id:document.getElementById('environment').value,name,value})});if(!r.ok)failed++}document.getElementById('import-status').textContent=failed?'Imported with '+failed+' invalid lines.':'Imported securely.';await load()}if(token)load();
</script></body></html>`;
